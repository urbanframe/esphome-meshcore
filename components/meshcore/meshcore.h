...
#endif
  // Declared unconditionally (not nested in either #ifdef above): whichever
  // of SX126xListener/SX127xListener actually got inherited above (it can be
  // either, or in principle both, depending on which radio headers the build
  // system's dependency graph pulls in — e.g. ESP-IDF only makes the radio
  // component you actually configured available, unlike Arduino's flatter
  // include tree), this single override satisfies its pure virtual on_packet
  // by identical signature. Leaving this inside an #ifdef MC_USE_SX126X guard
  // (as an earlier version of this file did) silently leaves the class
  // abstract whenever only sx127x is in play — that was a real bug, caught
  // by "new-expression of abstract class type" at the call site rather than
  // here, so don't reintroduce the nesting.
