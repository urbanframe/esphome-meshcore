    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_ON_ADVERT): automation.validate_automation(
                {cv.GenerateID(CONF_TRIGGER_ID): cv.declare_id(automation.Trigger)}
            ),
        }
...
async def to_code(config):
    ...
    for conf in config.get(CONF_ON_CHANNEL_MESSAGE, []):
        trigger = cg.Pvariable(conf[CONF_TRIGGER_ID], var.get_on_channel_message_trigger())
        await automation.build_automation(trigger, [(cg.std_string, "channel"), (cg.std_string, "text")], conf)
    for conf in config.get(CONF_ON_MESSAGE, []):
        trigger = cg.Pvariable(conf[CONF_TRIGGER_ID], var.get_on_message_trigger())
        await automation.build_automation(trigger, [(cg.std_string, "text")], conf)
    for conf in config.get(CONF_ON_ADVERT, []):
        trigger = cg.Pvariable(conf[CONF_TRIGGER_ID], var.get_on_advert_trigger())
        await automation.build_automation(trigger, [(cg.std_string, "pubkey"), (cg.std_string, "name")], conf)
